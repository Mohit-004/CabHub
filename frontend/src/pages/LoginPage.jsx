...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (result && result.success) {
      ...
      setError((result && result.message) || 'Login failed. Please check your credentials.');
    }
  };
  ...
  const handleQuickLogin = async (type) => {
    ...
    let result;
    if (type === 'passenger') {
      result = await login('passenger', { email: DUMMY_PASSENGERS[0].email, password: DUMMY_PASSENGERS[0].password });
      if (result && result.success) navigate('/passenger');
    } else if (type === 'driver') {
      result = await login('driver', { email: DUMMY_DRIVERS[0].email, password: DUMMY_DRIVERS[0].password });
      if (result && result.success) navigate('/driver');
    } else {
      result = await login('admin', { email: 'admin@cabhub.com', password: 'admin123' });
      if (result && result.success) navigate('/admin');
    }
    setLoading(false);
